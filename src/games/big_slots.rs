...
impl BigSlots {
    ...
    fn play_free_bet(
        ...
        loop {
            ...
            if *tiles_count.get(9).unwrap_or(&0) >= 3 {
                // scatter
                free_spins = 5;
            }
            for (index, tile_count) in tiles_count.iter_mut().enumerate() {
...
impl StatefulGameEng for BigSlots {
    ...
    fn continue_playing(
        ...
        let data: BigSlotsContinueData = serde_json::from_str(&bet.data)
            .map_err(|e| {
                error!(
                    "Error parsing BigSlots continue data `{:?}`: {:?}",
                    bet.data, e
                );
                e
            })
        ...
        let mut parsed_state: BigSlotsState = serde_json::from_str(&state.state)
            .map_err(|e| {
                error!("Error parsing BigSlots state`{:?}`: {:?}", &state.state, e);
                e
            })
            .ok()?;
        ...
        if data.use_free_spins {
            parsed_state.free_spins -= 1;
            let (game_fields, total_win, multipliers, total_win_per_tumble, free_spins) =
                self.play_free_bet(random_numbers, state.amount);
            parsed_state.free_spins += free_spins;
            parsed_state.game_fields = Some(game_fields);
            parsed_state.total_win += total_win;
            parsed_state.multipliers = Some(multipliers);
            parsed_state.total_win_per_tumble = Some(total_win_per_tumble.clone());

            if parsed_state.free_spins > 0 {
                return Some(GameResult {
                    total_profit: total_win,
                    outcomes: Vec::with_capacity(0),
                    profits: total_win_per_tumble.clone(),
